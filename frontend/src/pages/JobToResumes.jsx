...
export default function JobToResumes() {
  ...
  async function run() {
    ...
    setError(null);
    try {
      const { matches } = await api.matchJobToResumes(text, topK);
      setMatches(matches);
    } catch (e) {
      setError(e.message);
    } finally {
  ...
  return (
    <div>
      <header className="mb-8 flex items-center gap-2">
        <Users size={20} className="text-[var(--color-signal-mid)]" />
        <div>
          <h1 className="text-2xl font-semibold">Find Resumes for Job</h1>
          <p className="text-[var(--color-text-dim)] text-sm mt-1">Same fixed pipeline, run in the reverse direction.</p>
        </div>
      </header>
      ...
      <div className="flex items-center gap-4 mt-4">
        ...
        >
          {loading ? "Searching…" : "Find Resume Matches"}
        </button>
      </div>
        ...
        <div className="mt-8 space-y-3">
          ...
          {matches.map((m, i) => (
            <MatchCard
              ...
              assessment={m.analysis?.overall_assessment}
            />
          ))}
        </div>
      )}
